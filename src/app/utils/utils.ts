  ...
  files.forEach((file) => {
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        const slug = path
          .relative(contentBasePath, filePath)
          .replace(/\.mdx?$/, "")
          .replace(/\\/g, "/");
...
export function getAdjacentPages(currentSlug: string, sortType: SortType = "section") {
  try {
    ...
    const sections = Array.from(sectionMap.entries()).map(([section, pages]) => ({
      section,
      pages,
    }));
    ...
    sortedSections.forEach(({ section, pages }) => {
      ...
      const folders = Array.from(folderMap.entries()).map(([folder, folderPages]) => ({
        folder,
        pages: folderPages,
      }));
    ...
    let nextPage =
      currentIndex < sidebarOrderedPages.length - 1 ? sidebarOrderedPages[currentIndex + 1] : null;
