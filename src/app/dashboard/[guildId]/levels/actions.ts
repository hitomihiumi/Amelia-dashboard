...
export async function updateLevelsSettings(guildId: string, formData: FormData): Promise<GuildActionState> {
    try {
        ...
        const economy = JSON.parse(economyRaw as string) as GuildSchema['economy']['income']['level_up'];

        if (!levels || typeof levels !== 'object') return { ok: false, error: "Invalid levels data format." };
        if (!economy || typeof economy !== 'object') return { ok: false, error: "Invalid economy data format." };
        ...
        if (typeof levels.message?.delete !== 'number' || levels.message.delete < 0 || levels.message.delete > 60) {
        ...
        if (levels.level_roles && typeof levels.level_roles === 'object') {
            ...
            for (const [levelStr, roleId] of Object.entries(levels.level_roles)) {
                ...
                if (isNaN(level) || level <= 0 || level > 1000) {
                    return { ok: false, error: `Invalid level (${levelStr}) for role rewards. Must be between 1 and 1000.` };
                }
