...
export function CommandsFrom({
    ...
    const handleRestrict = (cmd: string, roles: string[]) => {
        setPerms((prev) => {
            ...
            const nextCommands = { ...prev.commands, [cmd]: { ...normalizedData, roles: normalizedData.roles.filter((r) => r.type === 'allow')  } };
            roles.forEach((r) => {
    ...
    const handleUnrestrict = (cmd: string, roles: string[]) => {
        setPerms((prev) => {
            const normalizedData = normalizePermission(prev.commands[cmd], cmd);
            const nextCommands = { ...prev.commands, [cmd]: { ...normalizedData, roles: normalizedData.roles.filter((r) => r.type === 'deny')  } };
            roles.forEach((r) => {
    ...
    return (
        <Column>
            <CommandAccordion
                title={
                    <Row gap={'12'}>
                        <Text variant="body-strong-s" onBackground={'brand-weak'}>/setting</Text>
            ...
            >
                <Column gap="12">
                    <RoleSelect
                        ...
                        selectedRole={perms.commands["setting"]?.roles ? perms.commands["setting"].roles.filter((r) => r.type === 'deny').map((r) => r.id) : []}
                        setSelectedRole={(role) => handleRestrict("setting", Array.isArray(role) ? role : [role])}
                    ...
                    <RoleSelect
                        ...
                        selectedRole={perms.commands["setting"]?.roles ? perms.commands["setting"].roles.filter((r) => r.type === 'allow').map((r) => r.id) : []}
                        setSelectedRole={(role) => handleUnrestrict("setting", Array.isArray(role) ? role : [role])}
                        options={guildRoles.map((role) => ({
                            label: <RolePill roleColor={role.color} label={role.name} />,
                            value: role.id,
                        }))}
