...
export async function discordAutoSetupTempVoice(
  ...
  { ok: true; categoryId: string; triggerChannelId: string } | { ok: false; error: string }
> {
  const bot = process.env.DISCORD_BOT_TOKEN?.trim();
  if (!bot) {
    return {
      ok: false,
      error: "DISCORD_BOT_TOKEN is not configured. Please set it in the environment variables.",
    };
  }
  ...
  let category = channels.find((c) => c.type === CHANNEL_TYPE_CATEGORY && c.name === CATEGORY_NAME);
  ...
  let trigger = channels.find(
    (c) =>
      c.type === CHANNEL_TYPE_VOICE && c.name === TRIGGER_VOICE_NAME && c.parent_id === category.id,
  );
